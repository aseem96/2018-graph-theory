...
If the degree of any vertex is 2, the three unconnected vertices form an independent set of size 3.  Therefore, in order to have an independent set of less than 3, every vertex needs to have a degree of at least 3.  If we build a 3-regular graph with 6 vertices, it will turn out as follows:\\
We start by connecting $a$ to $b, c,$ and  $d$. $b, c,$ or $d$ cannot be adjacent, since that would form a clique of size 3 with $a$. In so doing, we create an independent set of size 3 with $b, c, $ and $d$.\\
In general, a vertex v in a G has at least 3 neighbor or 3 non neighbors. By symmetry (if we complement G , we still have to prove the same statement), we may assume that x has at least 3 neighbors. If any pair of these vertices are connected, then with x we have 3 mutual connections (clique), but if no pair of neighbors of x is connected, then the neighbors of x are three non-neighbors (independent set).

According to Ramsey's Theorem, the value of $R(3,3)$ is ${(3+3-2) \choose (3-1)}$, or ${4 \choose 2} = 6$, meaning a graph with a guarnateed idependent set of size 3 or a clique of size 3 must have a $V(G) \geq 6$ \\
